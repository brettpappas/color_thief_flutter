...
var _sigbits = 5, _rshift = 8 - _sigbits, _maxIterations = 1000, _fractByPopulations = 0.75;
...
class VBox {
  ...
  List<int> avg([bool force = false]) {
    if (_avg == null || force == true) {
      var ntot = 0.0, mult = 1 << 8 - _sigbits, rsum = 0.0, gsum = 0.0, bsum = 0.0, hval, i, j, k, histoindex;
  ...
  contains(pixel) {
    var rval = pixel[0] >> _rshift, gval = pixel[1] >> _rshift, bval = pixel[2] >> _rshift;
...
List<VBox?> _medianCutApply(List<int?> histo, VBox vbox) {
  ...
  var rw = vbox.r2 - vbox.r1 + 1, gw = vbox.g2 - vbox.g1 + 1, bw = vbox.b2 - vbox.b1 + 1, maxw = _PV.max([rw, gw, bw]);
    ...
    for (int i = vbox.r1; i <= vbox.r2; i++) {
      ...
      for (j = vbox.g1; j <= vbox.g2; j++) {
        for (k = vbox.b1; k <= vbox.b2; k++) {
          index = _getColorIndex(i, j, k);
          sum += histo.elementAt(index) == null ? 0 : histo.elementAt(index)!;
  ...
  doCut(color) {
    ...
    for (int i = vboxdim1; i <= vboxdim2; i++) {
      ...
      if (partialsum_i != null && partialsum_i > total / 2) {
        ...
        while (_safeGetArray(partialsum, d2) == null || _safeGetArray(partialsum, d2) == 0) {
          d2++;
        }

        count2 = _safeGetArray(lookaheadsum, d2);

        while ((count2 == 0) && (_safeGetArray(partialsum, d2 - 1) != null && _safeGetArray(partialsum, d2 - 1) != 0)) {
          count2 = _safeGetArray(lookaheadsum, --d2);
        }
...
CMap? quantize(List<List<int>> pixels, int maxcolors) {
  ...
  iter(lh, target) {
    ...
    while (niters < _maxIterations) {
      ...
      var vboxes = _medianCutApply(histo, vbox), vbox1 = vboxes[0], vbox2 = vboxes[1];
